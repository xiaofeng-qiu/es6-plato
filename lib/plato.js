...
var defaultParserOptions = require('./parser-options');
...
function inspect(files, outputDir, options, done) {
	...
	var parserOptions = options.parserOptions ? options.parserOptions : defaultParserOptions;
		...
		files.forEach(function reportEachFile(file) {
			...
			if (options.recurse && fs.statSync(file).isDirectory()) {
				...
				_.each(reporters, function processReporter(reporter, name) {
					...
					try {
						report[name] = reporter.process(source, flags[name], report.info, parserOptions);
					} catch (e) {
						error = true;
